...
def measure_Lyman_continuum(df, colwave="rest_wave", colfnu="rest_fnu"):
    ...
    if df[colwave][0] > Lyc[0] :
        ...
        f_Lyc = jrr.util.bootstrap_val_confint( df[df[colwave].between(*Lyc)][colfnu], np.median, alpha=0.1) # Median flux in Ly cont
        f_red = jrr.util.bootstrap_val_confint( df[df[colwave].between(*red)][colfnu], np.median, alpha=0.1)  # and at 1500A
...
#    ax2.xaxis.set_major_locator(ticker.MultipleLocator(tick_spacing))
    plt.ylim((df[colfnu].median()*-0.1), jrr.util.robust_max(df[colfnu]))
...
print "#Currently printing 90% confidence intervals, bc util.bootstrap_val_confint(alpha=0.1)"
...
for row in df_indy.itertuples():
    print "DEBUGGING", type(row.Index)
    plt.annotate(row.Index, xy=(row.fnu_1500, row.fnu_Lyc), xycoords='data')
...
#plt.annotate(df_indy.ind, xy=(df_indy['fnu_1500'],  df_indy['fnu_Lyc']/df_indy['fnu_1500']), xycoords='data')
