...
f.write(aslatex)  # Might be useful for paper, who knows
f.close()
...
fig = plt.figure(figsize=figsize)  # start a new figure
not_stack = df[~df['filename'].str.contains('tack') & ~df['filename'].str.contains('chuck')]  # First, exclude stack
...
plt.scatter(bytZ.index.get_level_values(1), bytZ.index.get_level_values(0), s=((bytZ.frac_light.values/sum_to_norm)**2)*scatscale, color='blue', edgecolors='k')
...
plt.scatter( np.ones_like(ptsize)*30, np.linspace(1.7,2,num=len(ptsize)), s=((ptsize/sum_to_norm)**2*scatscale), color='blue',  edgecolors='k')
