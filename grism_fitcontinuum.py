...
Updating July 2021 to work on the Sunburst Arc G141 grism spectra.
'''

#### Setup  #########   By hand RUN THIS FOR EACH DIRECTORY, BOTH GRISMS
which_grism = "141" #  "102" #  
which_dirnum = 0 # Step through the dirs by hand (if more than 1 dir per run)  
...
wdir3 = home + '/SCIENCE/Lensed-LBGs/Planck_Arc/IR_grisms/1dspec/'
#
#              0X                      1                       2                                 3X         
#alldirs = (wdir1 + '1Dsum/', wdir1 + '1Dbyclumps/', wdir1 + '1D_complete_images_A2_A3/', wdir2 + '1Dsum/') # first run   # S1723
...
ocontdir = contdir + 'Restorecont/' # If _cont.fits file was missing, and I regenerated by backing it out of MWdr.txt file
...
extra_header = "# Milky Way dereddening (MWdr) has been applied by grism_fitcontinuum.py, E(B-V)="
...
if Paste_spectra_continuua :
    if   '1723' in grismdir : galname = 's1723'
    elif '2340' in grismdir : galname = 's2340'
    elif 'Planck' in grismdir : galname = 'sunburst'
    else : raise Exception("Do not recognize galname.  Cannot retrieve MW EBV")
    targ_df = jrr.MW_EBV.get_MW_extinctions()
    EBV = targ_df.loc[galname]['EBV'] 
    print("Debugging, using EBV", EBV)
    for specinfile in thefiles :
        root = re.sub('.txt', '', specinfile)
        ...
        # Figure out where the hell the continuum file is.  If can't find it in main, look for regererated file in Restorecont/
        if(os.path.exists(contdir + contfile)):      fullpathcontfile =  contdir + contfile
        elif (os.path.exists(ocontdir + contfile)):  fullpathcontfile = ocontdir + contfile
        else : raise Exception("Cannot find a continuum file", contdir + contfile)
        print("Proceeding with", contfile)
        data_in, header = fits.getdata(fullpathcontfile, header=True)
        ...
        head3 = str(EBV) + " for target " + galname + "\n"
        jrr.util.put_header_on_file('temp', grism_header+extra_header+head3,  contdir + wcontfile)
