...
labels = ('PSZ0441', 'SPT0310', 'SPT2325', 'SPT0356', 'SPT0142', 'planckarc_h9')  #
...
plotorder = ('PSZ0441',  'SPT0310', 'SPT0142', 'SPT2325', 'SPT0356', 'planckarc_h9')   # from bottom to top
need_HST = ('PSZ0441', 'SPT0142', 'SPT0356')  
...
nudge  = 1.1
key = from_meg[-1]
plt.plot(sp[key].rest_wave, (sp[key].rest_fnu / sp[key].rest_fnu_autocont) - nudge, label=key, linestyle='steps-mid')  # Plot S21226
...
for ii, key in enumerate(plotorder):
    ...
    if key == "planckarc_h9" : altlab = "Sunburst Arc"
    else :                     altlab = key
    ...
    if key in need_HST:
        plt.plot(df[key]['wave'], df[key]['tempy'] + ii*scalef, label=key, linestyle='steps-mid', lw=1.8, color='k')
        plt.annotate(plotlab, xy=(xy1-labelshift,ii*scalef+0.), xycoords='data', fontsize=18, fontweight='bold')
    else :
        plt.plot(df[key]['wave'], df[key]['tempy'] + ii*scalef, label=key, linestyle='steps-mid', lw=1.5)
...
nudge = ii +  7
plt.plot(sp[key].rest_wave, (sp[key].rest_fnu / sp[key].rest_fnu_autocont) + nudge, label=key, linestyle='steps-mid')
plt.plot(sp[key].rest_wave, (sp[key].rest_fnu_s99model / sp[key].rest_fnu_autocont) + nudge, label='_nolegend_', color='b')
...
plt.annotate(what_to_plot, xy=(0.6,0.01), xycoords='axes fraction', fontsize=20, fontweight='bold',horizontalalignment='right')
...
plt.yticks([])
plt.tight_layout()
fig.savefig(what_to_plot + "_C28extended.pdf")
