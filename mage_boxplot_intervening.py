...
if   batch == "batch1" :
    doublet_file = homedir + "/Dropbox/MagE_atlas/Contrib/Intervening/Doublet_search/Results_16Feb2018/found_doublets_SNR4_JRRedit.txt"
elif batch == "batch23" :
    doublet_file = homedir + "/Dropbox/MagE_atlas/Contrib/Intervening/Doublet_search/Results_21Feb2019/found_doublets_batch3_SNR4_JRRedit.txt"

trans_file = homedir + "/Dropbox/MagE_atlas/Linelists/MINE/interven.lst"
...
def make_boxplots_of_doublets(doublet_file=doublet_file) :
    ...
    #for row in foo.itertuples():  # TEMP, only run a few doublets, for debugging
        if row.flag == "y" :
            print "Working on intervening absorber at z=", row.zz, " in ", row.gal, row.pointing
            ...
            extra_label = row.gal + " " + row.pointing + " Intervening absorber at z="+ str(np.round(row.zz, decimals=5))
...
the_pdf = "intervening_doublets_boxplots_batch3.pdf"
pp = PdfPages(the_pdf)
