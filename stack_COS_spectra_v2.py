...
import astropy.convolution, astropy.constants
...
def get_COS_specres(infile='../Chisholm16_Spectral_Resolution.txt') :
    colnames = ('galname', 'Resl_kms')
    Rlist = pandas.read_table(infile, comment="#", names=colnames, delim_whitespace=True)
    A_c_kms = astropy.constants.c.to('km/s').value
    Rlist['R'] = A_c_kms / Rlist['Resl_kms']
    Rlist.set_index('galname', inplace=True)
    return(Rlist)
...
def blur_the_spectra(df, intermed_wave, new_wave, R1=1.4E4, R2=3300.) :  # Convolve and rebin the COS spectra to simulate MagE data.
    ...
    for this in df.keys() : #for each spectrum dataframe
        ...
        # This function rebins to critically sampled, then convolves w Gaussian, then rebins again
        # Start by rebinning to an intermediate wavelength array. nyquist=2.2.
...
def blur_the_spectra2(df, new_wave, Rlist, zzlist, R1=1.6E4, R2=3300.) :  # Convolve and rebin the COS spectra to simulate MagE data.
    idf = {}  ; ndf = {}  # intermediate and new data frames
    for this in df.keys() : #for each spectrum dataframe
        # Input is oversampled spectrum with high spectral resolution, as listed in Rlist
        # This function rebins to critically sampled, then convolves w Gaussian, then rebins again
        # Start by rebinning to an intermediate wavelength array. nyquist=2.2.
        (galname, grating, redshift) =  from_filename_get_props(this, zzlist)
        if galname in Rlist.index   :  RR = Rlist.loc[galname]['R']
        else                     :  RR = R1 ;  print "Warning,", this, "not found in Rlist, using default R=", R1
        intermed_wave = jrr.spec.make_wavearray_constant_resoln(1145, 1790, RR, 2.2)  # Nyquist sample to the measured resolution!  
        # Intermediate wavelength array has pixels to keep R=constant. It is critically sampled.
        idf[this] = pandas.DataFrame(data=pandas.Series(intermed_wave), columns=('obswave',))
        idf[this]['flam']    = jrr.spec.rebin_spec_new(df[this]['obswave'], df[this]['flam'],  intermed_wave)
        idf[this]['flam_u']  = jrr.spec.rebin_spec_new(df[this]['obswave'], df[this]['flam_u'], intermed_wave)
        idf[this]['badmask'] = jrr.spec.rebin_spec_new(df[this]['obswave'], df[this]['badmask'], intermed_wave)
        # Now, convolve w Gaussian, assuming destination is also a critically-sampled array
        kern_fwhm = np.sqrt((RR/R2)**2 - 1.) #convolve using the MEASURED actual COS resolution, rather than theoretical
        kern = astropy.convolution.Gaussian1DKernel(gaussian_fwhm_to_sigma * kern_fwhm)
        idf[this]['smooth']   = astropy.convolution.convolve(idf[this]['flam'].as_matrix(),   kern, boundary='fill', fill_value=np.nan)
        idf[this]['smooth_u'] = astropy.convolution.convolve(idf[this]['flam_u'].as_matrix(), kern, boundary='fill', fill_value=np.nan)
        # Should we be doing something smarter here with smooth_u?
        ...
        ndf[this]['badmask'] = ndf[this]['badmask'].astype(bool)  #nans will flag, as will partial values that were rebinned        
    prepare_spectra(idf) ; prepare_spectra(ndf)
    return(idf, ndf)  # idf = intermediate_df for debugging;  ndf = new df, rebinned to MagE resoln
...
R14000_wavear  = jrr.spec.make_wavearray_constant_resoln(1145, 1790, 1.4E4, 2.2)  # desired wavelength array for COS, nyquist sampled at max measured R
R3300_wavear   = jrr.spec.make_wavearray_constant_resoln(1145, 1790, 3300,  2.2) # appropriate output wavelength array for simulated-MagE stack, avgR
...
Rlist = get_COS_specres()     # Get the actual spectral resolns that JC measured from MW absorption lines 
...
(idf, df2) = blur_the_spectra2(df,  R3300_wavear, Rlist, zzlist, R1=1.4E4, R2=3300.)
...
print "About to start stacking..."

# STACKING STARTS HERE

# Make stack, with output array nyquist sampled for COS
(stacked, nf, nf_u) =  jrr.spec.stack_spectra(df, colwave='rest_wave', colf='rflam_norm', colfu='rflamu_norm', colmask='stackmask', output_wave_array=R14000_wavear)
stacked_output = "stacked_COS_spectrum_R1.4E4.csv"
stacked.to_csv(stacked_output, index=False)

# Make stack, with spectral resolution matched to MagE
(stacked2, nf2, nf_u2) =  jrr.spec.stack_spectra(df2, colwave='rest_wave', colf='rflam_norm', colfu='rflamu_norm', colmask='stackmask', output_wave_array=R3300_wavear)
stacked_output = "stacked_COS_spectrum_R3300.csv"
stacked2.to_csv(stacked_output, index=False)
print "Done stacking.  Now, lets make plots"
...
plt.close("all")
ax = stacked.plot(x='rest_wave', y='fweightavg', linewidth=0.5, color='k', drawstyle="steps-post", label='Native COS R')
stacked.plot(x='rest_wave', y='fweightavg_u', linewidth=1, color='grey', ax=ax, drawstyle="steps-post")
plt.plot(stacked.rest_wave, stacked.Ngal / stacked.Ngal.max(), color='green', linewidth=1)
stacked2.plot(x='rest_wave', y='fweightavg', linewidth=0.5, color='blue', drawstyle="steps-post", ax=ax, label='MagE avgR')
