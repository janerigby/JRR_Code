...
def draw_ERS_box(ax, ERSwin, ymin, ymax) :
    ax.fill( [ERSwin[0], ERSwin[1], ERSwin[1], ERSwin[0]], [ymin, ymin, ymax, ymax], 'grey', alpha=0.2, edgecolor='k')
    return(0)
...
for ii, thiswin in enumerate(windows) :
    ax1.plot(thiswin, (offsets[ii], offsets[ii]), label=names[ii], color=color[ii], lw=3)
    print "Primary target", names[ii], "window duration:", thiswin[1] - thiswin[0]
    if names[ii] != noL :
        ax1.annotate(names[ii], xy=(midpoint_date(*thiswin), 0.05+offsets[ii]), xycoords='data', ha='center')
...
for ii, thiswin in enumerate(awindows) :
    print "Alt target", anames[ii], "window duration:", thiswin[1] - thiswin[0]
    ax2.plot(thiswin, (aoffsets[ii], aoffsets[ii]), label=anames[ii], color=acolor[ii], lw=3)
    if anames[ii] != noL :
        ax2.annotate(anames[ii], xy=(midpoint_date(*thiswin), 0.05+aoffsets[ii]), xycoords='data', ha='center')
