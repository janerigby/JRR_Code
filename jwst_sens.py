...
def add_annotations():
    ...
    plt.annotate("MIRI", xy=(12,2E-17), color='k', xycoords='data', fontsize=fs2, alpha=0.5)
    plt.annotate("NIRSpec MOS", xy=(0.6,0.6E-18), color='orange', xycoords='data', fontsize=fs2)
    plt.annotate("& IFU", xy=(2., 0.6E-18), color='green', xycoords='data', fontsize=fs2)
...
def add_annotations_lowres():
    ...
    plt.annotate("MIRI", xy=(12,3E-5), color='k', xycoords='data', fontsize=fs2, alpha=0.5)
...
def grab_niriss_wfss():
    ...
    filename = 'niriss-wfss-line-sens.txt'
    df = pandas.read_csv(ndir + filename, delim_whitespace=True, comment='#')
    df['sens_cgs'] = df['Sensitivity'] * 1E-20 * 1E3   # sensitivity is in units of (10^-20 W/m^2)
    return(df)
...
plt.ion()
pp = PdfPages("jwst_sensitivity_jrigby.pdf")  # output
plt.close("all")
...
for ii, mode in enumerate(modes) :
    ...
    subset = df_sens[thismode].loc[df_sens[thismode]['configs'].astype(str).str.contains('w') &  \
                                       ~df_sens[thismode]['configs'].astype(str).str.contains('w2')]  # Skip W2
    # above is only W filters, but not W2
    print(subset.head())
    plt.plot(subset['wavelengths'], subset['lim_fluxes']*1E-3, label=labels[ii], color='black', marker='o', linestyle='-', alpha=0.3)
...
plt.text(11., 3E-7, "JWST MIRI", color='grey', fontsize=fs3)
plt.text(8.8, 4.5E-5, "Spitzer", color='orange', fontsize=fs3)
pretty_plot()

# Add post-commissioning sensitivities for NIRCam and NIRISS
...
plt.plot(subset_W.wavelength, subset_W.limiting_sensitivity_Jy, marker='o', color='blue')
#plt.plot(subset_M.wavelength, subset_M.limiting_sensitivity_Jy, marker='o', color='aqua')  # medium band NIRCam
plt.plot(df_postlaunch_niriss.wavelength, df_postlaunch_niriss.limiting_sensitivity_Jy, marker='o', color='red')
pp.savefig()
plt.show()
...
for specmode in specmodes:
    ...
    for index, row in df_specsens[thismode].iterrows() :     # Go row by row, for each grating, order, etc
        if 'miri' in thismode :
            plt.plot(row.wavelengths, row.PSSL, color='k', alpha=0.5)
            ...
            if row.configs['filter'] in just_plot_grisms:  # only plot predicted grism sens for wide filters!
                plt.plot(row.wavelengths[indices], row.PSSL[indices], color='blue', alpha=0.5)
                #plt.annotate(row.configs['filter'], (np.median(row.wavelengths), row.PSSL.min()), fontsize=10, color='blue')
        elif 'nirspec' in thismode:
            if 'ifu' in thismode:       plt.plot(row.wavelengths, row.PSSL, color='green', alpha=0.2)  
            elif 'msa' in thismode:     plt.plot(row.wavelengths, row.PSSL, color='orange', alpha=0.2)
        elif 'niriss' in thismode:
            threshold = 2
            indices = np.where(row.PSSL < threshold * row.PSSL.min())
            plt.plot(row.wavelengths[indices], row.PSSL[indices], color='red', alpha=0.4)
        else:
...
threshold = 5 # Don't plot the edges of the filter curve, just where it has sensitivity
for df1 in (df_ifs, df_mos) :
    for key in df1.keys():
        ...
        plt.plot(subset.wavelength *1E6, subset.PSSL, color=color) 

df_nircamgrism = load_nircam_grism_postlaunch() #post-commissioning nircam sensitivities
threshold = 1.4 # Don't plot the edges of the filter cuve, just where it has sensitivity
for key in df_nircamgrism.keys():
    df = df_nircamgrism[key]
    subset = df.loc[ (df['line_flux_E-18']  < threshold * df['line_flux_E-18'].min())]
    plt.plot(subset.wavelength, subset['line_flux_E-18'] * 1E-18, color='blue')  # data is in cgs, unit of 10^-18 erg/s/cm^2.
    #plt.annotate(key, (np.median(subset.wavelength), subset['line_flux_E-18'].min() *1E-18), fontsize=10, color='blue')

# post-commissioning niriss wfss sensitivity
df_niriss_wfss = grab_niriss_wfss()
for thisfilter in df_niriss_wfss.Filter.unique():
    subset = df_niriss_wfss.loc[df_niriss_wfss.Filter == thisfilter]
    plt.plot(subset.Wavelength, subset.sens_cgs, color='red')
...
for specmode in specmodes:
    ...
    for index, row in df_specsens[thismode].iterrows() :     # Go row by row, for each grating, order, etc
        if 'miri' in thismode :
            plt.plot(row.wavelengths, row.lim_fluxes *1E-3, color='k', alpha=0.5)
        elif specmode == 'nircam_wfgrism' :
            threshold = 1.4 
            ...
            if 'ifu' in thismode:       plt.plot(row.wavelengths, row.lim_fluxes *1E-3, color='green', alpha=0.2)  
            elif 'msa' in thismode:     plt.plot(row.wavelengths, row.lim_fluxes *1E-3, color='orange', alpha=0.2)
        elif 'niriss' in thismode:
            threshold = 2
            indices = np.where(row.lim_fluxes < threshold * row.lim_fluxes.min())
            plt.plot(row.wavelengths[indices], row.lim_fluxes[indices] *1E-3, color='red', alpha=0.4)
        else:
...
for key in df_nircamgrism.keys():
    ...
    plt.plot(subset.wavelength, subset['cont_flux_uJy'] * 1E-6, color='blue')  # convert from microJy to Jy
...
for df1 in (df_ifs, df_mos) :
    for key in df1.keys():
        ...
        plt.plot(subset.wavelength *1E6, subset.data, color=color) 

# post-commissioning NIRISS sensitivity -- Need Steph to send this
...
# Eichi, Fengwui's caveats about grism?
