...
def bpass_ages_setup() :  # Create a pandas dataframe of ages and column names ('coln'), as in BPASS v2.1 manual
    df_age = pandas.DataFrame([ 10**(6+0.1*(n-2)) for n in range(2,52)], columns=('age',))
...
df_bpass = pandas.read_table(bpassdir + filename, delim_whitespace=True, columns=(['wave'] +  df_age['colname'].tolist()))
